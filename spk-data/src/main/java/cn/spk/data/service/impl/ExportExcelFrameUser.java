...
public class ExportExcelFrameUser {
    /***
     * 构造方法
     */
    public ExportExcelFrameUser() {
     ...
     */
    public static void excelExport(List<FrameUser> dataList, Map<String, String> titleMap, String sheetName, HttpServletResponse response) {
        // 初始化workbook
...
//            out.close();
     ...
     * @param titleMap 表头信息
     */
    private static void createContentRow(List<FrameUser> dataList, Map<String, String> titleMap) {
        try {
            int i = 0;
            for (FrameUser frameUser : dataList) {
                HSSFRow textRow = sheet.createRow(CONTENT_START_POSITION + i);
                HSSFCell useridcell = textRow.createCell(0);
                useridcell.setCellValue(frameUser.getUserid());
                HSSFCell usernamecell = textRow.createCell(1);
                usernamecell.setCellValue(frameUser.getUsername());
                HSSFCell passwdecell = textRow.createCell(2);
                passwdecell.setCellValue(frameUser.getPasswd());
                HSSFCell createdatecell = textRow.createCell(3);
                createdatecell.setCellValue(new SimpleDateFormat("yyyy-MM-dd").format(frameUser.getCreatedate()));
                HSSFCell phonecell = textRow.createCell(4);
                phonecell.setCellValue(frameUser.getPhone());
                HSSFCell deptnamecell = textRow.createCell(5);

                deptnamecell.setCellValue(frameUser.getDeptname());
                i++;
            }
